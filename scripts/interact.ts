...
async function main() {
  ...
  const balances = async (label: string) => {
    const b1 = await publicClient.readContract({ 
      address: getAddress(TOKEN) as `0x${string}`, 
      abi, 
    ...
    const b2 = await publicClient.readContract({ 
      ...
      abi, 
      functionName: "balanceOf", 
      args: [getAddress(acct2)] 
    });
    console.log(`${label} | Deployer: ${formatUnits(b1 as bigint, 18)} CAMP | Acct2: ${formatUnits(b2 as bigint, 18)} CAMP`);
  };
  ...
  const tx1 = await wallet.writeContract({
    ...
    args: [getAddress(acct2), parseUnits("100", 18)],
  ...
  const tx2 = await wallet.writeContract({
    ...
    abi,
    functionName: "transfer",
    args: [getAddress(acct2), parseUnits("50", 18)],
  ...
  const tx3 = await wallet.writeContract({
    ...
    args: [getAddress(acct2), parseUnits("25", 18)],
